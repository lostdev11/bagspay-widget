...
interface UseMerchantResolutionResult {
  resolvedAddress: PublicKey | null
  isResolving: boolean
  error: string | null
  retry: () => void
}
 ...
 */
export function useMerchantResolution(
  input: string | null | undefined,
  connection: Connection
): UseMerchantResolutionResult {
  const [resolvedAddress, setResolvedAddress] = useState<PublicKey | null>(null)
  ...
  const resolveMerchant = useCallback(async (merchantInput: string, signal: AbortSignal) => {
    ...
    try {
      ...
      if (isValidSolDomain(trimmedInput)) {
        try {
          const resolved = await resolveAddress(trimmedInput, connection)
  ...
  }, [connection, isValidBase58Address])
